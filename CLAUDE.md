...
- **Jogo da Imagem's photos are real Pexels photos, downloaded once and self-hosted** in `public/photos/`, no CSP exception needed (`img-src` never changed). [imageWords.js](src/data/imageWords.js) maps each word to a local `/photos/<slug>.<ext>` file; the running app never calls the Pexels API and never sees the API key that resolved the original photo URLs (it was used transiently in a throwaway script, never committed). Swapping a photo means downloading a new file into `public/photos/` and repointing that one `icon` field — there's no live search wired into the app. An earlier version of this feature hotlinked `https://images.pexels.com` directly and opened an `img-src` exception for it; that was reverted in favor of downloading the files, so don't re-add that exception without checking this is still true.
