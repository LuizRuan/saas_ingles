...
- **Deliberate, narrow exception for `img-src`:** the Jogo da Imagem ([ImageQuiz.jsx](src/games/ImageQuiz/ImageQuiz.jsx)) loads real photos from `https://images.pexels.com`, requested explicitly by the user despite the self-hosting rule above. `img-src` names that **one** host — not a wildcard, not `default-src`. Each photo URL is resolved **once**, offline, via the Pexels Search API and frozen as a literal string in [imageWords.js](src/data/imageWords.js); the running app never calls the Pexels API itself and never sees the API key (it was used transiently in a throwaway script, never committed). If this game's image bank ever needs new entries, that one-time resolution step has to happen again with a fresh key — there's no live search wired into the app.
- **Fonts, everything else, still self-hosted.** The Pexels exception above is scoped to that one game's photos; don't treat it as license to start pointing other images/scripts/styles at third-party hosts. Continue to self-host per the rule at the top of this section.
